...
def test_grok_missing_key_safe_failure(test_db, test_user, test_page, test_product):
    """
    TEST 2: Grok with missing API key fails safely
    """
    rule = AutomationRule(id=2, user_id=test_user.id, page_id=test_page.id, name="Grok Missing Key Test", content_type="engagement", auto_post=False, scheduled_time="daily", is_active=True)
    test_db.add(rule)
    test_db.commit()
...
def test_grok_provider_wiring(test_db, test_user, test_page, test_product):
    """
    TEST 3: Grok provider selection and wiring
    """
    rule = AutomationRule(id=3, user_id=test_user.id, page_id=test_page.id, name="Grok Wiring Test", content_type="product_intro", auto_post=False, scheduled_time="daily", is_active=True)
    test_db.add(rule)
    test_db.commit()
