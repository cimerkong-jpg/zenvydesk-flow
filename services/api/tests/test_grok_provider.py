"""
Tests for Grok Provider Integration
Verifies Grok provider works correctly and doesn't break existing providers
"""
import pytest
...
def test_mock_provider_still_works(client, test_db, test_user, test_page, test_product):
    """
    TEST 1: Verify mock provider still works after adding Grok
    """
...
# TEST 2: Grok Missing Key Failure
def test_grok_missing_key_safe_failure(client, test_db, test_user, test_page, test_product):
    """
    TEST 2: Grok with missing API key fails safely
    """
    rule = AutomationRule(user_id=test_user.id, page_id=test_page.id, name="Grok Missing Key Test", content_type="engagement", auto_post=False, scheduled_time="daily", is_active=True)
    test_db.add(rule)
    test_db.commit()
    test_db.refresh(rule)
    ...
    try:
        ...
        print(f"\n✓ TEST 2 PASS: Grok missing key safe failure")
...
# TEST 3: Grok Provider Wiring
def test_grok_provider_wiring(client, test_db, test_user, test_page, test_product):
    """
    TEST 3: Grok provider selection and wiring
    """
    rule = AutomationRule(user_id=test_user.id, page_id=test_page.id, name="Grok Wiring Test", content_type="product_intro", auto_post=False, scheduled_time="daily", is_active=True)
    test_db.add(rule)
    test_db.commit()
    test_db.refresh(rule)
    ...
    try:
        ...
        assert data["status"] == "generation_failed"
        assert "Grok" in data["error"] or "grok" in data["error"].lower()
        
        print(f"\n✓ TEST 3 PASS: Grok provider wiring verified")
