...
def test_claude_missing_key_safe_failure(client, test_db, test_user, test_page, test_product):
    """
    TEST 2: Claude with missing API key fails safely
    """
    rule = AutomationRule(user_id=test_user.id, page_id=test_page.id, name="Claude Missing Key Test", content_type="engagement", auto_post=False, scheduled_time="daily", is_active=True)
    test_db.add(rule)
    test_db.commit()
    test_db.refresh(rule)
...
def test_claude_provider_wiring(client, test_db, test_user, test_page, test_product):
    """
    TEST 3: Claude provider selection and wiring
    """
    rule = AutomationRule(user_id=test_user.id, page_id=test_page.id, name="Claude Wiring Test", content_type="product_intro", auto_post=False, scheduled_time="daily", is_active=True)
    test_db.add(rule)
    test_db.commit()
    test_db.refresh(rule)
