...
def test_claude_missing_key_safe_failure(client, test_db, test_user, test_page, test_product):
    ...
    rule = create_automation_rule(
        ...
        user_id=test_user.id,
        page_id=test_page.id,
        name="Claude Missing Key Test",
        content_type="engagement",
    )

    with override_ai_settings(ai_provider="claude", ai_api_key=None):
        drafts_before = test_db.query(Draft).count()
...
def test_claude_provider_wiring(client, test_db, test_user, test_page, test_product):
    ...
    rule = create_automation_rule(
        ...
        user_id=test_user.id,
        page_id=test_page.id,
        name="Claude Wiring Test",
        content_type="product_intro",
    )

    with override_ai_settings(ai_provider="claude", ai_api_key="test_fake_key_for_wiring_test"):
        response = client.post(f"/api/v1/automation-runner/run/{rule.id}")
